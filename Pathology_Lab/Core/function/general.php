...
function count_laboratory()
{
    $sql="select count(*) from tbl_laboratory";
...
function count_pathologist()
{
    $sql="select count(*) from tbl_user where Type_Id=2";
    //$count=mysql_result(mysql_query($sql),0,0);
    
    return mysql_result(mysql_query($sql),0,0);
}
function count_confirm_reports()
{
    $sql="select count(*) from tbl_patient_checkup_details where Status_Id=4";
    //$count=mysql_result(mysql_query($sql),0,0);
